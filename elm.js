...
var $author$project$Main$NoResults = {$: 'NoResults'};
...
var $author$project$Validate$checkMultipleFixedConstraints = function (result) {
	var fixedByPerson = A3(
		...
		A2(
			...
			$elm$core$Dict$toList(result.data.preferences)));
	var peopleWithMultiple = $elm$core$Dict$toList(
		A2(
			$elm$core$Dict$filter,
			F2(
				function (_v3, courses) {
					return $elm$core$List$length(courses) > 1;
				}),
			fixedByPerson));
	var warnings = A2(
		$elm$core$List$map,
		function (_v2) {
			var person = _v2.a;
			var courses = _v2.b;
			var courseNames = A2($elm$core$List$map, $elm$core$Tuple$first, courses);
			var firstCourse = A2(
				$elm$core$Maybe$withDefault,
				'unknown',
				$elm$core$List$head(courseNames));
			return person + (' has multiple fixed constraints (' + (A2($elm$core$String$join, ', ', courseNames) + ('). Using only the first: ' + firstCourse)));
		},
		peopleWithMultiple);
	var cleanedPreferences = $elm$core$List$isEmpty(peopleWithMultiple) ? result.data.preferences : A2(
		$elm$core$Dict$map,
		F2(
			function (_v0, pref) {
				var person = _v0.a;
				var course = _v0.b;
				if (pref.fixed) {
					var _v1 = A2($elm$core$Dict$get, person, fixedByPerson);
					if (_v1.$ === 'Just') {
						var fixedCourses = _v1.a;
						var firstFixedCourse = A2(
							$elm$core$Maybe$withDefault,
							'',
							A2(
								$elm$core$Maybe$map,
								$elm$core$Tuple$first,
								$elm$core$List$head(fixedCourses)));
						return _Utils_update(
							pref,
							{
								fixed: _Utils_eq(course, firstFixedCourse)
							});
					} else {
						return pref;
					}
	...
	return _Utils_update(
		result,
		{
			...
			warnings: _Utils_ap(result.warnings, warnings)
		});
};
...
var $author$project$Validate$checkPeopleVsSlots = function (result) {
	var totalSlots = $elm$core$List$sum(
		$elm$core$Dict$values(result.data.courses));
	var numPeople = $elm$core$Dict$size(
		A3(
			$elm$core$List$foldl,
			F2(
				function (person, set) {
			...
			A2(
				$elm$core$List$map,
				$elm$core$Tuple$first,
				$elm$core$Dict$keys(result.data.preferences))));
	if (!_Utils_eq(numPeople, totalSlots)) {
		var warning = (_Utils_cmp(numPeople, totalSlots) < 0) ? ($elm$core$String$fromInt(totalSlots - numPeople) + (' course slots will be unfilled (' + ($elm$core$String$fromInt(numPeople) + (' people, ' + ($elm$core$String$fromInt(totalSlots) + ' total slots)'))))) : ($elm$core$String$fromInt(numPeople - totalSlots) + (' people will be unassigned (' + ($elm$core$String$fromInt(numPeople) + (' people, ' + ($elm$core$String$fromInt(totalSlots) + ' total slots)')))));
		return _Utils_update(
			result,
			{
				warnings: _Utils_ap(
...
var $author$project$Validate$checkRankingValidity = function (result) {
	var numCourses = $elm$core$Dict$size(result.data.courses);
	var allPeople = $elm$core$Dict$keys(
		A3(
			$elm$core$List$foldl,
			F2(
				function (person, set) {
			...
			function (person, _v1) {
				...
				var personPrefs = A2(
					...
					function (_v5) {
						var _v6 = _v5.a;
						var p = _v6.a;
						return _Utils_eq(p, person);
					},
					$elm$core$Dict$toList(prefs));
				var ranks = A2(
					$elm$core$List$map,
					function (_v4) {
						var pref = _v4.b;
				...
				var outOfRange = A2(
					$elm$core$List$filter,
					function (r) {
						return (r < 1) || (_Utils_cmp(r, numCourses) > 0);
					},
					ranks);
				var firstGap = $elm$core$List$head(
					A2(
						$elm$core$List$filter,
						function (n) {
							var countInRange = $elm$core$List$length(
								A2(
									$elm$core$List$filter,
									function (r) {
										return (r >= 1) && (_Utils_cmp(r, n) < 1);
									},
									ranks));
							return _Utils_cmp(countInRange, n) < 0;
						},
						A2($elm$core$List$range, 1, numCourses)));
				var fixedPrefs = (!$elm$core$List$isEmpty(outOfRange)) ? A2(
					$elm$core$Dict$map,
					F2(
						function (_v2, pref) {
							var p = _v2.a;
							if (_Utils_eq(p, person)) {
								var r = pref.rank;
								return ((r < 1) || (_Utils_cmp(r, numCourses) > 0)) ? _Utils_update(
									pref,
									{rank: numCourses + 1}) : pref;
							} else {
								return pref;
							}
						}),
					prefs) : ((!_Utils_eq(firstGap, $elm$core$Maybe$Nothing)) ? A2(
					$elm$core$Dict$map,
					F2(
						function (_v3, pref) {
							var p = _v3.a;
							if (_Utils_eq(p, person)) {
								var r = pref.rank;
								var numBetter = $elm$core$List$length(
									A2(
										$elm$core$List$filter,
										function (rank) {
											return _Utils_cmp(rank, r) < 0;
										},
										ranks));
								return _Utils_update(
									pref,
					...
					prefs) : prefs);
				var personWarnings = function () {
					if (!$elm$core$List$isEmpty(outOfRange)) {
						return _List_fromArray(
							[
								person + (' has out-of-range rankings (' + (A2(
								$elm$core$String$join,
								', ',
								A2($elm$core$List$map, $elm$core$String$fromInt, outOfRange)) + '). These will be treated as last place.'))
							]);
					} else {
						if (!_Utils_eq(firstGap, $elm$core$Maybe$Nothing)) {
				...
				return _Utils_Tuple2(
					fixedPrefs,
					_Utils_ap(warnings, personWarnings));
			}),
		_Utils_Tuple2(result.data.preferences, _List_Nil),
		allPeople);
	var cleanedPreferences = _v0.a;
	var newWarnings = _v0.b;
	return _Utils_update(
		result,
		{
			data: {courses: result.data.courses, preferences: cleanedPreferences},
			warnings: _Utils_ap(result.warnings, newWarnings)
		});
};
var $author$project$Validate$validate = function (inputData) {
	var result = {data: inputData, warnings: _List_Nil};
	return $author$project$Validate$checkRankingValidity(
		$author$project$Validate$checkMultipleFixedConstraints(
			$author$project$Validate$checkPeopleVsSlots(result)));
};
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'FileLoaded':
				var csvContent = msg.a;
				var _v1 = $author$project$CsvParser$parse(csvContent);
				if (_v1.$ === 'Ok') {
					var inputData = _v1.a;
...
var $author$project$Main$viewHeader = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Assign people to positions using '),
					A2(
					$elm$html$Html$a,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$href('https://en.wikipedia.org/wiki/Leximin_order'),
							$elm$html$Html$Attributes$target('_blank')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text('leximin optimization')
						])),
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$href('https://jefelino.github.io'),
							$elm$html$Html$Attributes$target('_blank')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text('Jeff Russell')
						])),
...
var $author$project$Main$viewInstructions = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('How to use this')
				])),
			A2(
			$elm$html$Html$p,
			_List_Nil,
			_List_fromArray(
				[
					$elm$html$Html$text('Upload a CSV file with each person\'s ranked preferences.')
				])),
			A2(
			$elm$html$Html$h3,
			_List_Nil,
			_List_fromArray(
				[
					$elm$html$Html$text('Example CSV')
				])),
			A2(
			$elm$html$Html$pre,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('example-code')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$text('Person C,        3,        2,        1\n'),
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text('The first row lists positions.')
						])),
					A2(
					$elm$html$Html$li,
					_List_Nil,
					_List_fromArray(
						[
							$elm$html$Html$text('The second row shows how many people can be assigned to each position.')
						])),
					A2(
					$elm$html$Html$li,
					_List_Nil,
					_List_fromArray(
						[
							$elm$html$Html$text('Each following row lists a person with their rankings (1 = top choice).')
						]))
				])),
			A2(
			$elm$html$Html$h3,
			...
			_List_fromArray(
				[
					$elm$html$Html$text('Constraints')
				])),
			A2(
			$elm$html$Html$ul,
			_List_Nil,
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('inline-code')
								]),
							_List_fromArray(
								[
									$elm$html$Html$text('*')
								])),
							$elm$html$Html$text(' to assign someone to a specific position')
						])),
					A2(
					$elm$html$Html$li,
					_List_Nil,
					_List_fromArray(
						[
							...
							$elm$html$Html$text(' to prevent assignment to a position')
						]))
				])),
			A2(
			$elm$html$Html$pre,
...
var $author$project$Main$viewDistribution = F2(
	function (dist, copyState) {
		...
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('distribution')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text('Preference Distribution')
								])),
							A2(
							$elm$html$Html$div,
							_List_fromArray(
								...
								[
									...
									_List_fromArray(
										[
											...
											$elm$html$Html$Events$onClick($author$project$Main$CopySummaryToClipboard),
...
var $author$project$Main$viewSingleAssignment = F3(
	function (copyState, index, assignment) {
		...
		var labelText = (index > 0) ? ('Variation ' + $elm$core$String$fromInt(index)) : 'Assignments';
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
...
var $author$project$Main$viewWarnings = function (warnings) {
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Error parsing CSV')
							])),
						A2(
						$elm$html$Html$p,
						_List_Nil,
...
var $author$project$Main$viewStyles = A3(
	$elm$html$Html$node,
	'style',
	_List_Nil,
	_List_fromArray(
...
var $author$project$Main$viewUploadSection = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$elm$html$Html$text('📁 Upload CSV File')
				]))
		]));
